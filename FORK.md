...
| Safe-Print topbar badge | `src/components/TheTopbar.vue` | Always-visible warning `v-chip` shown **only when** `save_variables.variables.safe_print_mode == 1` — the flag silently forces the legacy step/dir path and once cost a 5.5h print because nothing prominent surfaced it. Tooltip explains; click routes to the dashboard (Print Mode panel). Invisible in the normal OFF state. |
...
| Numeric input UX | `src/components/inputs/MiscellaneousSlider.vue`, `src/components/inputs/NumberInput.vue` | Unified commit model on both shared numeric inputs (fan/output % AND Machine/Extruder/PA/etc.): **Tab & Enter commit**, **click-away resets** (cancel a mistype), **Arrow Up/Down step + apply live** (debounced). A `tabCommitting` flag stops the post-Tab blur from clobbering the just-committed value. Original blur-commit was submitted upstream (PR #2554 / issue #2555) but **declined** (maintainer wanted blur=cancel, Tab=commit — except Tab didn't actually commit in their code); this is that intended model, fork-only. |
