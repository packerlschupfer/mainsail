...
| Panel registration | `src/store/variables.ts`, `src/pages/Dashboard.vue`, `src/components/mixins/dashboard.ts`, `src/store/gui/getters.ts`, `src/locales/en.json` | `build-sheet` + `fans` registered in `allDashboardPanels`, mapped in Dashboard.vue, icons in dashboard mixin, gated in `getAllPossiblePanels`, i18n strings. |
